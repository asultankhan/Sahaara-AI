...
def match_opportunities(city, education, skills, goal):
    ...
    for index, row in results.iterrows():
        ...
        if str(city).lower() in row_text or "all" in row_text:
            score += 2
            reasons.append("Location suitable")

        if str(education).lower() in row_text:
            score += 2
            reasons.append("Education matches")

        if str(skills).lower() in row_text:
            score += 2
            reasons.append("Skills match")

        results.loc[index, "match_score"] = score
        results.loc[index, "match_reason"] = ", ".join(reasons)
